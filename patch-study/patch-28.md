...
1. File: `include/linux/sched/ext.h`
   Hunk: `@@ -49,12 +49,15 @@ enum scx_dsq_id_flags {`
   Before
   ```text
   * Dispatch queue (dsq) is a simple FIFO which is used to buffer between the
   * scheduler core and the BPF scheduler. See the documentation for more details.
...
2. File: `init/init_task.c`
   Hunk: `@@ -102,7 +102,7 @@ struct task_struct init_task __aligned(L1_CACHE_BYTES) = {`
   Before
   ```text
   .dsq_node	= LIST_HEAD_INIT(init_task.scx.dsq_node),
   ```
...
3. File: `kernel/sched/ext.c`
   Hunk: `@@ -638,6 +638,7 @@ enum scx_enq_flags {`
   Before
   ```text
...
4. File: `tools/sched_ext/include/scx/common.bpf.h`
   Hunk: `@@ -31,6 +31,7 @@ static inline void ___vmlinux_h_sanity_check___(void)`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
   ...
   void scx_bpf_dispatch_vtime(struct task_struct *p, u64 dsq_id, u64 slice, u64 vtime, u64 enq_flags) __ksym;
   ```
5. File: `tools/sched_ext/scx_simple.bpf.c`
   Hunk: `@@ -2,11 +2,20 @@`
   Before
   ```text
   * A simple global FIFO scheduler. It also demonstrates the following niceties.
   ...
   * While very simple, this scheduler should work reasonably well on CPUs with a
   ...
   ```
6. File: `tools/sched_ext/scx_simple.c`
   Hunk: `@@ -17,8 +17,9 @@ const char help_fmt[] =`
   Before
   ```text
   "Usage: %s [-v]\n"
   ```
