...
1. File: `Documentation/scheduler/index.rst`
   Hunk: `@@ -20,6 +20,7 @@ Scheduler`
   Before
   ```text
   ...
   sched-ext
   ```
2. File: `Documentation/scheduler/sched-ext.rst`
   Hunk: `@@ -0,0 +1,314 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ...
   ==========================
   ```
3. File: `include/linux/sched/ext.h`
   Hunk: `@@ -1,5 +1,7 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ...
   *
   ```
4. File: `kernel/Kconfig.preempt`
   Hunk: `@@ -156,4 +156,5 @@ config SCHED_CLASS_EXT`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
...
6. File: `kernel/sched/ext.h`
   ...
   *
   ```
7. File: `tools/sched_ext/README.md`
   Hunk: `@@ -0,0 +1,258 @@`
   Before
   ```text
   -
