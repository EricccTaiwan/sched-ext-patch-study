...
**View on Lore**: [https://lore.kernel.org/all/20240618212056.2833381-11-tj@kernel.org](https://lore.kernel.org/all/20240618212056.2833381-11-tj@kernel.org)
...
1. File: `Makefile`
   Hunk: `@@ -1355,6 +1355,12 @@ ifneq ($(wildcard $(resolve_btfids_O)),)`
   Before
   ```text
...
2. File: `tools/Makefile`
   Hunk: `@@ -28,6 +28,7 @@ include scripts/Makefile.include`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
   ...
   ```
3. File: `tools/sched_ext/.gitignore`
   Hunk: `@@ -0,0 +1,2 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ...
   build/
   ```
4. File: `tools/sched_ext/Makefile`
   Hunk: `@@ -0,0 +1,246 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ...
   include ../build/Build.include
   ```
5. File: `.../sched_ext/include/bpf-compat/gnu/stubs.h`
   Note
   ```text
   File appears in diff stats but no explicit hunk was captured in this markdown block.
   ```
6. File: `tools/sched_ext/include/scx/common.bpf.h`
   Hunk: `@@ -0,0 +1,379 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ...
   * Copyright (c) 2022 Meta Platforms, Inc. and affiliates.
   ```
7. File: `tools/sched_ext/include/scx/common.h`
   Hunk: `@@ -0,0 +1,75 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ...
   * Copyright (c) 2023 Meta Platforms, Inc. and affiliates.
   ```
8. File: `tools/sched_ext/include/scx/compat.bpf.h`
   Hunk: `@@ -0,0 +1,28 @@`
   Before
   ```text
   (no notable removed lines in sampled hunk)
