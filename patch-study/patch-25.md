...
1. File: `include/linux/sched/ext.h`
   Hunk: `@@ -98,13 +98,15 @@ enum scx_kf_mask {`
   Before
   ```text
   __SCX_KF_RQ_LOCKED	= SCX_KF_DISPATCH |
   ```
...
2. File: `kernel/sched/ext.c`
   Hunk: `@@ -110,6 +110,32 @@ struct scx_exit_task_args {`
   Before
   ```text
...
3. File: `kernel/sched/ext.h`
   Hunk: `@@ -24,6 +24,8 @@ DECLARE_STATIC_KEY_FALSE(__scx_switched_all);`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
...
4. File: `kernel/sched/sched.h`
   Hunk: `@@ -737,6 +737,7 @@ struct scx_rq {`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
   ...
   bool			cpu_released;
   ```
5. File: `tools/sched_ext/include/scx/common.bpf.h`
...
7. File: `tools/sched_ext/scx_qmap.c`
   Hunk: `@@ -112,9 +112,9 @@ int main(int argc, char **argv)`
   Before
   ```text
   printf("stats  : enq=%lu dsp=%lu delta=%ld deq=%"PRIu64"\n",
   skel->bss->nr_dequeued);
