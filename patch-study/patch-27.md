...
1. File: `include/linux/sched/ext.h`
   Hunk: `@@ -129,6 +129,9 @@ struct sched_ext_entity {`
   Before
   ```text
   ...
   #endif
   ```
2. File: `kernel/Kconfig.preempt`
   Hunk: `@@ -135,7 +135,7 @@ config SCHED_CORE`
   Before
   ```text
   depends on BPF_SYSCALL && BPF_JIT && !SCHED_CORE
   ```
...
3. File: `kernel/sched/core.c`
   Hunk: `@@ -169,7 +169,10 @@ static inline int __task_prio(const struct task_struct *p)`
   Before
   ```text
   return MAX_RT_PRIO + MAX_NICE; /* 120, squash fair */
   ```
...
4. File: `kernel/sched/ext.c`
   Hunk: `@@ -344,6 +344,24 @@ struct sched_ext_ops {`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
   ...
   * @a: task A
   ...
   Note
   ```text
   Additional hunks continue in this file (18 more section(s)).
   ```
5. File: `kernel/sched/ext.h`
   Hunk: `@@ -70,6 +70,11 @@ static inline const struct sched_class *next_active_class(const struct sched_cla`
   Before
   ```text
   (no notable removed lines in sampled hunk)
   ```
...
6. File: `tools/sched_ext/scx_qmap.bpf.c`
   ...
   * - Core-sched support.
   ...
   Note
   ```text
   Additional hunks continue in this file (9 more section(s)).
   ```
7. File: `tools/sched_ext/scx_qmap.c`
   Hunk: `@@ -112,9 +112,10 @@ int main(int argc, char **argv)`
   Before
   ```text
   printf("stats  : enq=%lu dsp=%lu delta=%ld reenq=%"PRIu64" deq=%"PRIu64"\n",
   skel->bss->nr_reenqueued, skel->bss->nr_dequeued);
